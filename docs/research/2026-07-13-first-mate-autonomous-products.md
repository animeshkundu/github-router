...
Note: the full `bun test` suite has a pre-existing colgrep/code-search contention flake (`tests/isolated/unified-code-search.test.ts` "forced lexical family" — passes deterministically in isolation, can ✗ under full-suite parallelism from colgrep sidecar provisioning). Unrelated to this change; not chased here.
