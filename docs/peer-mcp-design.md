...
| opus_critic | claude-opus-4.7-1m-internal (enterprise; else claude-opus-4-7) | /v1/messages | xhigh (default; thinking.budget=24000) | 30-90s on real reviews — fits inside SSE-streamed `/mcp` |

**opus_critic model selection**: `activePersonas()` (`src/routes/mcp/handler.ts`) resolves opus_critic's model at call time — it prefers the 1M-context Opus variant (`claude-opus-4.7-1m-internal`, ≈936K-token prompt window, `restricted_to: ["enterprise"]`) when the live catalog carries it (matched by `/opus-4\.7.*1m/i`), and falls back to the 200K `claude-opus-4-7` (≈168K prompt) otherwise. The canonical Copilot-Enterprise deployment gets the 1M window, so opus_critic and codex_critic (gpt-5.5, ≈922K) are the two big-window peers that can take a large artifact whole.
