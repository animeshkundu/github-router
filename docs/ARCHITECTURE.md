...
- `ANTHROPIC_MODEL` = Anthropic-published dashed slug (defaults to `claude-opus-5` or first available fallback). NOT the Copilot-internal slug — see "Subcommand-level fallback chains" in §"Model Resolution" for why
...
| `claude` | `/v1/messages` | `claude-opus-5` (Anthropic dashed slug and exact Copilot catalog id; single base slug, native 1M) | `claude-opus-4-8` → `claude-opus-4-7` → `claude-opus-4-6` |
