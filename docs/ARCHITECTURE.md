...
Note that the `claude` chain uses **Anthropic-published dashed slugs**. The default `claude-opus-5.5` is also an exact Copilot catalog-id match; older fallback `claude-opus-4-8` is translated to Copilot's dotted `claude-opus-4.8`. `ANTHROPIC_MODEL` must hold a slug that Claude Code's `/model` UI registry recognizes, and the proxy's resolver handles any needed translation on every `/v1/messages` request via the body rewrite at `src/routes/messages/handler.ts:326`. The `codex` chain uses Copilot slugs directly because Codex CLI's bundled catalog uses Copilot-style IDs.
...
- `ANTHROPIC_MODEL` = Anthropic-published dashed slug (defaults to `claude-opus-5.5` or first available fallback). NOT the Copilot-internal slug — see "Subcommand-level fallback chains" in §"Model Resolution" for why
...
|---|---|---|---|
| `codex` | `/v1/responses` | `gpt-6-sol` | `gpt-5.5` → `gpt-5.4` → `gpt-5.3-codex` → `gpt-5.2-codex` |
| `claude` | `/v1/messages` | `claude-opus-5.5` (Anthropic dashed slug and exact Copilot catalog id; single base slug, native 1M) | `claude-opus-4-8` → `claude-opus-4-7` → `claude-opus-4-6` |
