...
The compressor backend is picked at startup from the live Copilot catalog by walking a static fallback chain: `gpt-5.4-mini` → `claude-haiku-4-5` → `gemini-3.5-flash`. First entry present AND advertising `tool_calls` support wins. The picked id is logged. `gemini-3.5-flash` is intentionally last — it early-stops on forced tool-calls (the same reliability issue that pushed the worker engine off it), so it is only selected when neither mini nor haiku is present. When NONE of the three is in the catalog, the compound tools are dropped from `tools/list` AND fail `tools/call` with -32601 (gate at `browserCompoundToolsEnabled()` in `src/lib/mcp-capabilities.ts`, mirroring the existing `workerToolsEnabled` / `standInToolEnabled` pattern). The L1 primitives are unaffected — `browser_act` ref-mode keeps working without the compressor.
