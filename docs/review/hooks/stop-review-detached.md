# Hook 7: detached `Stop` review (advisory, gpt-6-sol)
...
| Backing model | `gpt-6-sol` at `thinking: "high"`, via the read-only `workers/review` MCP tool (`src/internal-stop-review.ts:146`) |
