...
### 2c. `PROMPT_SCOPE_SYSTEM`: the gpt-6-sol scope-inference system prompt (non-trivial, v2): `prompt-submit-hook.ts:88-99`

Not injected into the model directly; it steers a single gpt-6-sol call whose OUTPUT (a <=120-word grounded scope/goal note) is injected. It instructs the scoping model to restate the user's own ask as one measurable objective, reference the most relevant files by name, and NOT invent new requirements. It adds the `/gh-research` + `/gh-orchestrate` line only when the task is large/cross-cutting.

### 2d. `framePendingFindings`: prior-turn review findings wrapper (v2): `prompt-submit-hook.ts:133-140`

> ADVISORY — independent review of your PREVIOUS change (NON-AUTHORITATIVE): an independent gpt-6-sol reviewer flagged the following. Evaluate each on its merits — fix the real ones, and ignore any wrong one with a one-line reason. You are NOT obligated to act on these.
...
- Triviality split via `isNonTrivialPrompt` (`:38-47`): true if the prompt is >=280 chars, matches a build/change verb regex (`implement|build|refactor|migrate|fix|debug|…`), or a multi-file scope phrase.
  ...
  - **Non-trivial** → parallel lexical+semantic `mcp__search__code` (limit 10) → one gpt-6-sol `/v1/responses` call at effort `low` → grounded scope note; the whole enrichment is timeout-bounded (22s default, per-call 8s search / 18s infer) with a fail-open to `PROMPT_STEER_GOAL` (`:207-248`).
...
- The MODEL-CALL enrichment is correctly gated to non-trivial prompts (`isNonTrivialPrompt`), so a `git commit -m fix` pays no gpt-6-sol latency. Good.
...
This hook is ALREADY the closest thing in the suite to the intelligent-hook pattern: a deterministic trigger (every prompt) + a bounded-inference gate (the 22s-bounded gpt-6-sol scope call) that decides how much context to add, with a fail-open to the regex goal. It honors all four non-negotiables: fail-open on timeout/error, advisory phrasing, bounded latency, widen-not-narrow (it only adds context).
