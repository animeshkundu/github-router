...
Governing lens: raise the floor, never nerf. This surface makes the four non-Claude
...
| Where set | `src/lib/server-setup.ts:763-775` (enable) + `seedGatewayModelCache` `:449-479` (seed) + `clearGatewayModelCache` `:489-499` |
| Seeded models | `nativeSelectableModelsInCatalog()` — the catalog-present subset of `gpt-5.5`, `gpt-5.3-codex`, `gemini-3.5-flash`, `gemini-3.1-pro-preview` (`:378-408`) |
| Guard | presence-based, checks BOTH parent env AND in-function `vars` (`:768-769`) |
...
- The **cache-READ path** applies NO id filter (the `/^(claude|anthropic)/i` filter lives
  ...
  untouched (`src/lib/server-setup.ts:410-448`).
...
(`:773-775`). Selecting a seeded row sends the real Copilot id, which `resolveModel`
...
path appends, and the seed contains ONLY the four non-Claude ids).
...
**Non-regression is structural, not probabilistic** (`src/lib/server-setup.ts:749-753`): the
...
display_name?}]}`), verified against build 2.1.201 (`:414-417`). If a future build changes the
...
- **[Suggestion]** `src/lib/server-setup.ts:414-448` — the seed is coupled to Claude Code's
...
- **[Suggestion]** The `NATIVE_NON_CLAUDE_MODELS` list (`:378-386`) is hardcoded. A newer
...
seed carries only the four non-Claude ids. Conditional enable + clear-on-empty + both-sides
