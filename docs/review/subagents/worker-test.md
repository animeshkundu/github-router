...
| Subagent's OWN model | inherited (Claude); the WORKER runs `IMPLEMENT_DEFAULT_MODEL` = `gpt-6-sol` at high (test shares the implement default — repo CLAUDE.md) |
...
The "independent, tries to break, does not touch the implementation" framing is exactly the floor-raiser: it keeps test authorship decorrelated from implementation (which trends gpt-6-sol) — an honest adversarial test pass. The `worktree` option isolates the test run. Right balance.
