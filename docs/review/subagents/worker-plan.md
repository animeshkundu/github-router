...
| Subagent's OWN model | inherited (Claude); the WORKER runs `PLAN_DEFAULT_MODEL` = `claude-opus-5.5` (exact catalog id) at high by default; callers can restore a higher tier per call or via `worker_defaults` |
