...
| Subagent's OWN model | inherited (Claude); the WORKER runs `PLAN_DEFAULT_MODEL` = `claude-opus-4.8` (dotted catalog id) at xhigh — "planning is the highest-leverage step, so it gets the strongest model" (repo CLAUDE.md) |
