...
**Fast launch profile.** A `-m fast` Luna-led launch profile uses exactly `Explore` (Luna/high), `Plan` (Sol/high), `general-purpose` (Luna/max), `implementer` (Gemini 3.8 Flash/high), and `reviewer` (Grok 4.6/medium), each carrying fixed model and effort frontmatter. Capitalized `Explore` replaces fast `scout`; standard launches retain lowercase `scout` and the built-in Explore. The profile also drops standard peer critics, coordinator, core filesystem worker dispatchers, and orchestrate/decide groups. Its balanced delegation graph allows the lead to invoke all five roles, `Plan` to invoke `reviewer` or `Explore`, `implementer` to invoke `reviewer`, and `reviewer`/`Explore` are terminal. In Fast mode, only the lead and `Plan` can call Oracle when required; `reviewer` and other native subagents cannot. In-session PreToolUse hooks remove invocation-level model overrides so fixed frontmatter remains authoritative. See the "Fast launch profile" section of [`../../default-models.md`](../../default-models.md).
