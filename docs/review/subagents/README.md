...
Two shapes of subagent: RELAY shims (the five critics, codex-implementer, and the six worker dispatchers, which run on the inherited lead model and forward through MCP) and the eight NATIVE agents. `implementer` prefers the OpenAI frontier chain; `reviewer` and `brainstorm` prefer Gemini then that frontier chain (`reviewer` deliberately NOT sharing `implementer`'s model, or a review of implementer-produced work would be one model checking its own output); `scribe` prefers gpt-5.6-terra then the frontier chain. When a preferred chain misses, those four omit `model:` and inherit the lead. `scout`, `implementer-fast`, and the two `generic-*` catch-alls are omitted when their qualifying chains miss, because inheriting the lead would defeat their cost purpose. `scout` walks Luna → Gemini Flash and enforces a 1M floor; it is dropped when neither qualifies. All four conditional natives require both `tool_calls` and 1M context. `scout` and `brainstorm` carry a read-only `tools:` allowlist, while `implementer`, `implementer-fast`, `reviewer`, `scribe`, and the two catch-alls omit `tools:` and inherit the full toolset. The `worker-*` dispatchers are separately pinned to `mcp__<workersKey>__*` only, so they cannot recurse.
