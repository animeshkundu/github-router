...
Max follows a layered routing contract. The resident digest holds only direct-versus-delegate, independent-versus-dependent, and evidence rules. Its compact awareness summary names the native roster once. The mirrored awareness block is the live gate-aware inventory. Agent and peer descriptions own their what/when/when-not boundaries; their prompt bodies own scope, deliverables, verification, and stop conditions. Native `reviewer` is for repository/runtime-backed verification, while cold-start peers review only supplied artifacts. The Max coordinator has peer MCP tools only and chooses the smallest sufficient set of distinct lenses instead of counting votes. Max Advisor remains lead-only, transcript-aware, optional, and non-binding; it can challenge framing but is not independent verification. See [`MAX-PROMPT-EVAL.md`](MAX-PROMPT-EVAL.md) for the preregistered evaluation.
...
**Fast launch profile.** A `-m fast` Gemini 3.8 Flash-led launch profile uses exactly `Explore` (Luna/high), `Plan` (Sol/high), `general-purpose` (Luna/max), `implementer` (Gemini 3.8 Flash/high), and `reviewer` (Claude Sonnet 5 1M/xhigh), each carrying fixed model and effort frontmatter. Capitalized `Explore` replaces fast `scout`; standard launches retain lowercase `scout` and the built-in Explore. The profile also drops standard peer critics, coordinator, core filesystem worker dispatchers, and orchestrate/decide groups. Its balanced delegation graph allows the lead to invoke all five roles, `Plan` to invoke `reviewer` or `Explore`, `implementer` to invoke `reviewer`, and `reviewer`/`Explore` are terminal. In Fast mode, only the lead and `Plan` can call Oracle when required; `reviewer` and other native subagents cannot. In-session PreToolUse hooks remove invocation-level model overrides so fixed frontmatter remains authoritative. See the "Fast launch profile" section of [`../../default-models.md`](../../default-models.md).
