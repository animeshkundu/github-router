...
**Fast launch profile.** A `-m fast` Luna-led launch profile uses exactly `scout`/`implementer`/`reviewer`/`planner`/`critic`, each carrying fixed model and effort frontmatter. The profile also drops every other native, coordinator, worker dispatcher, and standard peer surface. Its compiled Task/Agent ACL is described in [critic.md](critic.md); keep this audit table profile-aware rather than assuming the standard roster applies unchanged. See the "Fast launch profile" section of [`../../default-models.md`](../../default-models.md).

The fast ACL allows the lead to invoke all five roles, `planner` to invoke `reviewer`, `scout`, or `critic`, and `implementer` to invoke `reviewer` or `critic`; `reviewer`, `scout`, and `critic` are terminal. Standard profile definitions and delegation remain unchanged.
