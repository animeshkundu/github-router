...
| `Explore` (native, fast only) | gpt-5.6-luna | capitalized read-only repository exploration role replacing fast `scout`; bare/default local accounting (provider 1.05M total / 922K max prompt / 128K max output) | exact fast-profile prerequisite: 1M `tool_calls`, high effort, Responses endpoint | [scout.md](scout.md) | Y |
| `Plan` (native, fast only) | gpt-5.6-sol | capitalized Sol-backed plan consultant and approver; `[1m]` accounting | exact fast-profile prerequisite: 1M `tool_calls`, high effort, Responses endpoint | (undocumented — fast role shares plan contract) | not yet reviewed |
| `critic` (native, fast only) | gemini-3.7-flash | fresh-context cross-lab challenge of plans, designs, diffs, and decisions | exact fast-profile prerequisite: 1M `tool_calls`, medium effort, chat endpoint | [critic.md](critic.md) | Y |
...
The fast-only `critic` native role is documented separately in [critic.md](critic.md). It is not part of the standard roster or standard delegation graph.

**Fast launch profile.** A `-m fast` Luna-led launch profile uses exactly `Explore`/`implementer`/`reviewer`/`Plan`/`critic`, each carrying fixed model and effort frontmatter. Capitalized `Explore` replaces fast `scout`; standard launches retain lowercase `scout` and the built-in Explore. The profile also drops every other native, coordinator, worker dispatcher, and standard peer surface. Its compiled Task/Agent ACL is described in [critic.md](critic.md); keep this audit table profile-aware rather than assuming the standard roster applies unchanged. See the "Fast launch profile" section of [`../../default-models.md`](../../default-models.md).

The fast ACL allows the lead to invoke all five roles, `Plan` to invoke `reviewer`, `Explore`, or `critic`, and `implementer` to invoke `reviewer` or `critic`; `reviewer`, `Explore`, and `critic` are terminal. Allowed dispatches remove invocation-level model overrides so fixed frontmatter remains authoritative. Standard profile definitions and delegation remain unchanged.
