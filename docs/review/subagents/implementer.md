...
It is one of eight native agents: `implementer`, `reviewer`, `brainstorm`, `scout`, `scribe`, `generic`, `generic-fast`, and `generic-cheap`. Unlike peer critics, it does the work in its own agent loop rather than relaying a request through an MCP persona.
