...
It is one of seven native agents: `implementer`, `implementer-fast`, `reviewer`, `brainstorm`, `scout`, `scribe`, and `general-purpose-fast`. Its scope replaces the former split between `qa-engineer` and `debugger`: assessment includes the investigation needed to establish what is wrong.
