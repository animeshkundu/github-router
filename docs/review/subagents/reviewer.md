...
It is one of five native agents: `implementer`, `reviewer`, `brainstorm`, `scout`, and `scribe`. Its scope replaces the former split between `qa-engineer` and `debugger`: assessment includes the investigation needed to establish what is wrong.
