...
| Subagent's OWN model | inherited (Claude); the WORKER runs `REVIEW_DEFAULT_MODEL` = `gemini-3.1-pro-preview` at high (cross-lab, decorrelated from the gpt-6-sol implementers — repo CLAUDE.md) |
...
Correctly positions worker-review as the "reviewer that reads the code itself" — complementary to the paste-only peer critics and the coordinator, not a replacement. The cross-lab default model (gemini) is a deliberate decorrelation from the gpt-6-sol implementers. Raises the floor without nerfing the critic path. Right balance.
