...
| Backing model (of the peer it routes to) | `gpt-6-sol` `/v1/responses` (`src/lib/peer-mcp-personas.ts:336-337`) |
...
The subagent is a relay; the graded critique is produced by the server-baked `CRITIC_BASE` running on gpt-6-sol.
...
- **Specific not vague — strong.** Names the model (gpt-6-sol), the lab-diversity value, the artifact types it fits (plans/designs/tradeoffs) and the one it does not (line-level diffs), with a redirect. A reader can route confidently.
- **Accurately previews the body — yes.** "Adversarial second opinion", "cross-lab", "Pass artifact verbatim" all map directly to `CRITIC_BASE` (adversarial critic, gpt-6-sol, cold-start-contract "paste a self-contained brief").
