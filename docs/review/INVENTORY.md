...
- **Fast profile is a separate native/MCP surface on `-m fast`.** A Luna-led (`gpt-5.6-luna`) profile drops the standard `implementer`/`reviewer`/`brainstorm`/`scribe`/`general-purpose-fast` roles, coordinator, worker dispatchers, and `workers`/`orchestrate` groups. It emits exactly `Explore`/`implementer`/`reviewer`/`planner`/`critic`, with fixed model/effort frontmatter; capitalized `Explore` replaces fast `scout`. `planner` may invoke `reviewer`/`Explore`/`critic`, `implementer` may invoke `reviewer`/`critic`, and reviewer/Explore/critic are terminal under a fast-only PreToolUse ACL that removes invocation-level model overrides. Fast Advisor is fixed to Gemini 3.7 Flash/high at both the client identity and proxy dispatch layers. The fast MCP surface keeps only `search`, `oracle`, and opt-in browser. Standard launches, including direct `-m gpt-5.6-luna`, are unchanged. See [`default-models.md`](../default-models.md) "Fast launch profile".
