...
- **Fast profile is a separate native/MCP surface on `-m fast`.** A Luna-led (`gpt-5.6-luna`) profile drops the standard `implementer`/`reviewer`/`brainstorm`/`scribe`/`general-purpose-fast` roles, coordinator, worker dispatchers, and `workers`/`orchestrate` groups. It emits exactly `Explore`/`implementer`/`reviewer`/`Plan`/`critic`, with fixed model/effort frontmatter; capitalized `Explore` replaces fast `scout`. `Plan` may invoke `reviewer`/`Explore`/`critic`, `implementer` may invoke `reviewer`/`critic`, and reviewer/Explore/critic are terminal under a fast-only PreToolUse ACL that removes invocation-level model overrides. Fast `Explore` and Grok `reviewer` intentionally use bare aliases/frontmatter ids for Claude Code's conservative/default local accounting; this does not claim either provider window is only 200K. Luna remains a required 1M catalog/startup prerequisite because it serves as lead and implementer. Fast Advisor is fixed to Gemini 3.7 Flash/high at both the client identity and proxy dispatch layers. The fast MCP surface keeps only `search`, `oracle`, and opt-in browser. Standard launches, including direct `-m gpt-5.6-luna`, are unchanged. See [`default-models.md`](../default-models.md) "Fast launch profile".
