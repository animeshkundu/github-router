...
### S3. Worker `thinking` field says "default high" but several modes default to xhigh

The generic `thinking` field text "Optional reasoning depth (default high)" was copy-pasted across worker modes, but the real per-mode defaults differ: plan = `xhigh` (`engine.ts:198`), explore = `xhigh` (`DEFAULT_THINKING`, `engine.ts:136`), review = `xhigh` clamped to high for the default Gemini model (`engine.ts:161`), implement/test = `xhigh`. A model reading "default high" under-reasons or needlessly passes `thinking:"xhigh"` to upgrade what is already xhigh.

- Affected tools: plan (`peer-mcp-personas.ts:1490-1493`), explore (`:1238`), review (`:1413`), and by inheritance implement/test.
