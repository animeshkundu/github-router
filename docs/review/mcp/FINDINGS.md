...
opus_critic now prefers `claude-opus-5.5`, whose single base slug is natively 1M. On catalogs without Opus 5 it falls back to `claude-opus-4.6-1m`, then `claude-opus-4-6`. Its allowed efforts include `xhigh`; `defaultEffort` remains `high` to preserve the prior latency profile.

### S5. Worker model defaults refreshed

Worker defaults now match `engine.ts`: review uses `gemini-3.1-pro-preview` (`xhigh`, clamped to high), explore uses `gpt-6-luna` at `high`, plan uses `claude-opus-5.5` at `high`, implement/test use `gpt-6-sol` at `high`, and browse uses `gpt-6-luna` at `high`. Per-call and session overrides can restore higher effort where supported. Explore/implement/review keep a free-string `model` override and document the recommended 1M sol/terra/flash ladder.
...
| workers | browse | Y | `gpt-6-luna`/high; dispatcher field mismatch tracked in S13 |
| workers | explore | Y | `gpt-6-luna`/high; free-string 1M override ladder documented (S5) |
| workers | implement | Y | No when-to-prefer clause vs `implementer` subagent + `codex_implementer` (S6) |
| workers | plan | Y | `claude-opus-5.5`/high; higher effort remains caller-selectable (S3/S5) |
...
- [x] A4 (S5). Worker model descriptions now match the live defaults: review `gemini-3.1-pro-preview`, explore `gpt-6-luna`/high, plan `claude-opus-5.5`/high, implement/test `gpt-6-sol`/high, and browse `gpt-6-luna`/high.
