...
| Backing model / endpoint | server-side fn `runWorkflowLive` (`src/lib/orchestration/run-workflow-live.ts:85`); internally spins the worker engine + cross-lab critics (gpt-6-sol `/responses`; gemini-3.1-pro-preview `/chat/completions`, claude-opus-4-6 `/chat/completions`) + real gate subprocesses via `liveExec` |
