...
- `default_model` (string, optional): "Model the GitHub cloud coding agent uses for this mission's tasks; defaults to gpt-6-sol, with gpt-5.5 as fallback."
...
- **Accuracy vs implementation.** Verified accurate:
  - `default_model` "defaults to gpt-6-sol (gpt-5.5 fallback)" matches `DEFAULT_CODEX_MODEL = "gpt-6-sol"` (first fallback `gpt-5.5`) (`src/lib/port.ts:155`), resolved via `resolveCloudAgentModel` (`tools.ts:245`, `task-model.ts:32-59`). An explicit invalid model throws FAST at input time inside the tool try/catch (`tools.ts:240-245`), so the "defaults to gpt-6-sol" wording (with gpt-5.5 fallback) plus fail-fast behavior are consistent.
...
No contradictions between description ↔ system prompt ↔ CLAUDE.md ↔ code. All four agree that `start_mission` is registration-only, gated by `agents`, defaults model to gpt-6-sol (gpt-5.5 fallback), and defers decomposition. The only cross-surface asymmetry is completeness: `plan_gate`/`ci_required` live only in the tool description + code, absent from the design doc.
...
Y — `start_mission`'s injected surface is correct, minimal, and consistent; the description accurately conveys registration semantics, the gpt-6-sol default (gpt-5.5 fallback), and the plan-gate/CI-required semantics, all verified against the controller. Single most important fix: document `plan_gate` and `ci_required` in `docs/first-mate-design.md:26-28` so the design doc stops trailing the shipped schema.
