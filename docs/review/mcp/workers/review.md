...
- **[Suggestion]** `docs/peer-mcp-design.md:365` is stale for worker defaults — "review `gpt-5.5`" (should be `gemini-3.1-pro-preview`) and adjacent "explore `gpt-5.4-mini`" (engine says `claude-sonnet-5`, `engine.ts:146`). The per-mode table `:359-361` and defaults line should be reconciled with `engine.ts` and the correct root `CLAUDE.md:133`. Adjacent to this tool; flag when fixing.
