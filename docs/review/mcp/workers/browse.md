...
| Backing model / endpoint | `gpt-5.6-luna` (`BROWSE_DEFAULT_MODEL`), high thinking (`BROWSE_DEFAULT_THINKING`); routed through the same Pi worker engine as explore/review/implement |
...
Gate detail (`browseAgentEnabled()`, `mcp-capabilities.ts:248-255`): requires `browserToolsEnabled()` (the `--browse` opt-in AND a supported browser on disk) AND the `gpt-5.6-luna` default present in the live catalog with a usable endpoint. Fires symmetrically at `tools/list` (`handler.ts:338`) and `tools/call` (`handler.ts:928-938`, -32601 on miss) — the same defense-in-depth as `worker`/`stand_in`.
