...
| Always-on? | yes (`requiresHttp: true`, no catalog gate — `claude-opus-4-6` assumed present on supported tiers) |
| Capability gate | none (`requiresGeminiCatalog` is undefined; `personasFor` never drops it) |
| Backing model / endpoint | `claude-opus-4-6` (resolved to `claude-opus-4.6-1m` when catalog carries it) `/v1/messages` |
| Write-capable | no |

Model resolution is dynamic: `activePersonas()` overrides the static `claude-opus-4-6` with `resolveOpusCriticModel()` (`src/routes/mcp/handler.ts:262-265`), which prefers a `claude-opus-4.6-1m` slug (`OPUS_1M_RE = /opus-4[.-]6-1m(?:$|-)/i`, `handler.ts:261`) and falls back to the 200K base slug. allowedEfforts `["low","medium","high"]`, defaultEffort `high` (`personas.ts:418-419`) — xhigh omitted because 4.6 advertises only `low|medium|high|max`.
...
- `effort` (string, optional; enum `low|medium|high`): per-persona effort tier.
...
Covering block: **peer-awareness** (marker pair `PEER_MARKER_OPEN`/`_CLOSE`, `claude-md-injection.ts:20-22`). `appendPeerAwarenessToMirroredClaudeMd` (`claude-md-injection.ts:653-663`) writes the exact `buildPeerAwarenessSnippet` output — so the mirrored CLAUDE.md carries the identical ``opus_critic` (Opus 4.7)`` string from surface 2b. No separate opus_critic text; the mirror and the `--append-system-prompt` share one source.

Checked-in root `CLAUDE.md` documents opus_critic in the peer-review paragraph ("`opus_critic` (Opus 4.7)"), matching the code snippet's string verbatim (both wrong — see findings). Root `CLAUDE.md:9` independently states the ground truth: "`claude-opus-4.6-1m` (opus_critic's pinned model)" and "`claude-opus-4.7-1m-internal` (stand_in's Opus row)" — so the repo's own canonical model table confirms opus_critic is 4.6, not 4.7.
...
- **Accurate & non-redundant**: the snippet correctly defers routing to the description ("Each tool's description explains its scope"), so it is non-redundant. But the parenthetical model tag `(Opus 4.7)` CONTRADICTS the tool description's `Opus 4.6` — the two surfaces disagree on the model version (see Finding 1).
...
| Awareness snippet / mirrored CLAUDE.md (`personas.ts:585`) | Opus 4.7 | ❌ |
| Subagent prompt `OPUS_CRITIC_BASE` (`personas.ts:322,324`) | Claude Opus 4.7 | ❌ |
| Window-guard hint (`handler.ts:575`) | Opus-4.7 1M ≈ 936K | ❌ |
| Design doc Phase B (`docs/peer-mcp-design.md:205`) | Opus 4.7 | ❌ |
| Design doc window-guard example (`docs/peer-mcp-design.md:203`) | opus_critic Opus-4.7-1M ≈936K | ❌ |
...
- `src/lib/peer-mcp-personas.ts:585` — snippet pushes ``"`opus_critic` (Opus 4.7)"``; model is `claude-opus-4-6` (`personas.ts:401`).
- `src/lib/peer-mcp-personas.ts:322` and `:324` — `OPUS_CRITIC_BASE` tells the subagent it is "running on Claude Opus 4.7". This is the subagent's own identity statement, so the peer critic asserts a false version about itself in its reply framing.
- `src/routes/mcp/handler.ts:575` — window-guard hint routes overflow to "`opus_critic` (Opus-4.7 1M ≈ 936K tokens)".
...
Fix: replace "Opus 4.7" → "Opus 4.6" at `personas.ts:585`, `personas.ts:322`, `personas.ts:324`, `handler.ts:575`, and `docs/peer-mcp-design.md:203,205`, and the peer-review sentence in root `CLAUDE.md`. Root cause is almost certainly a default-bump: these strings were written when the spawned-Claude default was 4.7 and opus_critic was "one minor behind"; the default moved to 4.8 and opus_critic stayed on 4.6, but the "4.7" labels were not swept.
...
- Default Opus is `claude-opus-4-8` (root `CLAUDE.md:119`, `docs/default-models.md`); opus_critic is 4.6. That is two minor versions behind (4.8 → 4.7 → 4.6).
