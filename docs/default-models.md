...
| `Explore` | `gpt-5.6-luna` | high | Broad read-only repository discovery; Claude Code uses its conservative/default local context accounting |
...
| `Plan` | `gpt-5.6-sol[1m]` | high | Plan consultant and approver after Luna drafts |
...
All five catalog models are mandatory, even though Gemini serves both the critic and Advisor. Startup fails with an actionable list rather than substituting a model or shipping a partial surface. Grok stays bare because its live limits are 500K total, 372K prompt, and 128K output. The bare fast `Explore` and `reviewer` ids intentionally use Claude Code's conservative/default local context accounting; bare does not claim that either provider window is only 200K.

### Evidence boundary for fast context accounting

The official [Copilot billing models and pricing page](https://docs.github.com/en/enterprise-cloud@latest/copilot/reference/copilot-billing/models-and-pricing) establishes the billing distinction between default context (up to 200K) and long context (above 200K), but it does not establish the exact context ceilings or Claude Code's local accounting for a role. The live Copilot `/models` catalog is authoritative for those fields on 2026-08-28: Grok 4.6 is 500K total / 372K max-prompt / 128K max-output; Luna is 1.05M / 922K / 128K; Sol is 1.05M / 922K / 128K; and Gemini 3.7 Flash is 1M / 936K / 64K. Fast `Explore` (Luna) and `reviewer` (Grok) intentionally use bare private aliases/frontmatter ids, which selects Claude Code's conservative/default local accounting rather than claiming a 200K provider window. Luna's 1M catalog/startup prerequisite remains load-bearing because Luna is both the fast lead and fast implementer; the Plan and critic roles retain `[1m]` accounting.

### Plan workflow

Luna performs the repository legwork and drafts. The lead then gives `Plan` a handcrafted evidence packet containing the goal, acceptance criteria, constraints, `file:line` and command/test evidence, settled decisions, the complete draft, and one focused question. `Plan` returns `APPROVE`, `REVISE`, or `NEED_MORE_CONTEXT`; implementation waits for `APPROVE`.

`Plan` has repository read/search/command tools so it can verify disputed citations or fill a narrow evidence gap. Its native delegation is ACL-scoped: it may call `reviewer`, `Explore`, or `critic`, but no other subagent. `implementer` may call `reviewer` or `critic`; `reviewer`, `Explore`, and `critic` are terminal. Capitalized `Explore` replaces the former fast `scout`; standard launches retain lowercase `scout` and the client's built-in Explore. These Task/Agent edges are enforced by a fast-only PreToolUse hook, which also removes invocation-level `model` overrides on every allowed dispatch so fixed role frontmatter wins. Plan's no-edit/no-plan-execution guidance and the lead's `APPROVE` gate remain prompt contracts, not a state machine. Bash remains a general execution primitive, so shell-launched nested processes are outside the in-session ACL.
...
Fast mode injects no github-router Stop-lifecycle automation: no structural Stop gate, detached Stop/plan review, ExitPlanMode auto-review, artifact auto-open, or review UserPromptSubmit hook. One concise instruction replaces it: obtain `Plan` approval before implementation; before declaring done, run the relevant build/tests and ask `reviewer` to verify.
...
The current fast/standard catalog derives `816700` because Luna/Sol bind at a 783.7K trigger; Opus/Sonnet/Gemini individually derive `828600`. The calculation minimizes the **complete expression**, not the prompt field alone, because output reserve participates. Missing/unusable limits omit that candidate; if no usable `[1m]` candidate remains, the variable is omitted rather than guessed. Fast `Explore` and Grok `reviewer` remain bare locally, so Claude Code uses its conservative/default accounting for those role ids even though the live provider metadata is larger.
