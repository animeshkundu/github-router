...
| `ANTHROPIC_DEFAULT_HAIKU_MODEL` | `claude-sonnet-5` | Seeded to Sonnet 5 (not a Haiku slug) to match the `ANTHROPIC_SMALL_FAST_MODEL` default — the cheap-tier pick lands on Sonnet 5, which is newer and cheaper than `claude-haiku-4-5`. Bare slug: the `[1m]` decoration lives only on the active default (`ANTHROPIC_MODEL` via `pickClaudeDefault`), never on picker tier rows. |
