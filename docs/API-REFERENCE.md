...
**Models**: gpt-6-sol, gpt-5.5, gpt-5.4, gpt-5.3-codex, gpt-5.2-codex, gpt-4.1, etc. The `codex` subcommand defaults to `gpt-6-sol` (fallback chain starts with `gpt-5.5` — see `src/lib/port.ts`).
...
**Models**: claude-opus-5.5 (single base slug, native 1M context), claude-opus-4.8 (single base slug, 1M context via `max_context_window_tokens`), claude-opus-4.7-1m-internal (enterprise-only), claude-opus-4.7, claude-opus-4.6-1m, claude-opus-4.6, claude-sonnet-4.6, etc. The `claude` subcommand sets `ANTHROPIC_MODEL=claude-opus-5.5` (Anthropic-published dashed slug and exact Copilot catalog id).
**Model resolution**: accepts both Anthropic dashed slugs and Copilot dotted slugs. `claude-opus-5.5` is an exact catalog match whose single base slug is already 1M-capable. `claude-opus-4-8` → `claude-opus-4.8`; `claude-opus-4-7` → `claude-opus-4.7-1m-internal` on enterprise or `claude-opus-4.7` on non-enterprise. `claude-opus-4.7-1m-internal` is accepted directly. `opus` → highest-version 1M-capable variant (Opus 5 first, then 4.8 base slug or the newest `-1m` sibling).
