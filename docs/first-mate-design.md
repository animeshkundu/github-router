...
- `mcp__first-mate__start_mission` — persist a mission: goal, repos, acceptance
  ...
  (the cloud coding agent's model for this mission's tasks; defaults to gpt-6-sol, with gpt-5.5 as the first fallback).
...
`gpt-6-sol` default (the same `DEFAULT_CODEX_MODEL` the codex launcher uses, with `gpt-5.5` as first fallback). The
...
`gpt-6-sol`, walking `DEFAULT_CODEX_MODEL_FALLBACKS` (starting with `gpt-5.5`) only when a catalog says the
...
and dispatch on the `gpt-6-sol` default, or the `gpt-5.5` fallback when needed.
