...
3. **`internal-stop-review.ts`** (new detached subcommand, register in `src/main.ts` + banner-suppress it): reads {session_id, cwd, diff, prompt, transcript_path, findings-path} from argv/stdin, calls `workers/review` via the client (read-only, model gpt-5.6-sol, the custom accountability prompt: judge the diff against the user's prompt + plan for wrong-spec / vacuous-tests / incompleteness; surface findings; do NOT author/run tests), writes the findings text to `fileFindingsStore`. It may run up to the worker wall-clock cap; it's detached so nothing waits on it.
