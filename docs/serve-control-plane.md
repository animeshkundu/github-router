...
- **Reasoning-effort / thinking picker is inert.** The façade rewrites model OPTIONS without an `effort.values` array, so `resolveClaudeEffort` sees `allowedEfforts=[]` and any UI effort selection is dropped; every run uses the binary default (non-Claude models → the shim's `high`). CloudCLI never sends an Anthropic `thinking` budget either.
...
- **`POST /api/system/update`** (CloudCLI's "update available" button) runs a **global** `npm install -g @cloudcli-ai/cloudcli@latest`, which mutates the user's global npm and never applies to serve's pinned router-owned install — passed through today; do not click it.
