...
8. **Run the wider suite.** Run `bun test` and compare pass/fail counts with the old pin. CI runs isolated tests separately, so report exclusions and any environmental failure accurately.
9. **Audit and clean up.** Diff the vendor tree against the exact upstream tag, permitting only documented divergences and the deliberate AI slice. Remove the temporary clone. Confirm git status contains no temporary or generated artifact before review/PR.
