...
|---|---|---|---|---|
| GPT-5.6 `prompt_cache_key` + `prompt_cache_options:{mode:"explicit",ttl:"30m"}` + content `prompt_cache_breakpoint:{mode:"explicit"}` | ✅ 200 | copilot-cli | `gpt56_explicit_cache_breakpoint` | Exact explicit-cache arm used only by router-owned GPT-5.6 reusable-prefix calls. This probe confirms Copilot ACCEPTS the shape; it does not measure a cost benefit — that is pending the live evidence harness (see `prompt-caching.md`). |
