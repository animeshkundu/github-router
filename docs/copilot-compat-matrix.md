...
| custom tool named `web_fetch` (`type:custom`) | ❌ 400 (Copilot denylists the name — NOT the proxy) | exploratory | `tooltype_web_fetch_custom_name_copilot_rejects` | The proxy's hosted-web_fetch gate matches the `type` slug only, never the tool NAME, so it forwards a custom tool named `web_fetch` unchanged. Copilot then rejects it itself with `400 "rejected tool(s): web_fetch"` (distinct from the proxy's hosted-web_fetch message) — proving the proxy did not over-match. |
...
| `output_config: {format: {type, schema}}` (Structured Outputs GA 2026-02-17 nested shape) | ✅ 200 (proxy strips `format`, injects schema as system prompt) | anthropic-docs | `output_config_format_ga_stripped` | GA moved the pre-GA flat `.schema`/`.type` under `.format`. Proxy reads the nested location (flat takes precedence for back-compat) so the schema intent survives via prompt injection; Copilot 400s on `output_config.*` other than `effort`. |
...
| `diagnostics: {previous_message_id}` (cache-diagnosis body field) | ✅ 200 (proxy strips) | anthropic-docs | `cache_diagnostics_stripped` | `cache-diagnosis-2026-04-07`. Copilot 400s on the unknown top-level field and has no cache-diagnostics backend; proxy strips the body field (the `cache-diagnosis-` beta header is dropped by the allowlist). No synthetic `cache_miss_reason` is generated; unknown response fields pass through untouched. |
