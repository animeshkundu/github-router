...
| Automatic fast Advisor with missing or wrong-endpoint Gemini catalog row | resolver fallback; explicit pin still wins | ✅ deterministic policy test | proxy-internal | `fast_advisor_endpoint_gate` | An ID-only Gemini check would silently route an unusable Advisor. Automatic selection requires Gemini's fixed Chat endpoint; an operator `GH_ROUTER_ADVISOR_MODEL` pin remains authoritative by design. |
