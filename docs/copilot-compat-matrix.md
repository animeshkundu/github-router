...
| Cheap family Advisor client identity | client `gpt-5.6-sol` (bare, no `[1m]`); upstream bare `gpt-5.6-sol`, Responses/high, transcript capped at `CHEAP_PROFILE_ADVISOR_CONTEXT_TOKENS` 200K | ✅ deterministic launcher + route tests | proxy-internal | `cheap_advisor_bare_pin` | Both cheap aliases pin the bare 200K slug (see `src/lib/cheap-advisor-client.ts`); identical to each other, unlike fast's `[1m]` Advisor pin. |
