...
| `gpt-5.4-mini` | (untested via this matrix) | exploratory | (none) | Browser-MCP inner compressor / extraction model — **chain head** (intent resolution for `act` / `observe` / `extract`; fallback chain `gpt-5.4-mini` → `claude-haiku-4-5` → `gemini-3.5-flash`; see [`docs/browser-mcp-design.md`](browser-mcp-design.md)). Plain chat-completions calls, no special request shape to pin here. |
| `gemini-3.5-flash` | (untested via this matrix) | exploratory | (none) | Browser-MCP inner compressor / extraction model — **last-resort fallback only** (demoted from chain head because it early-stops on forced tool-calls; selected only when neither `gpt-5.4-mini` nor `claude-haiku-4-5` is in the catalog). Plain chat-completions calls, no special request shape to pin here. |
