...
| `gemini-3.1-pro-preview` | ✅ 200 (`/v1/chat/completions` accepts `tools[]` + `reasoning_effort:"high"`) | exploratory | `shim_gemini31pro_messages` (+ shape carrier `worker_gemini_tools_reasoning`) | Model for the `gemini-critic` / `gemini-reviewer` personas and `stand_in`. (No longer the worker default — workers use `gpt-5.4-mini` explore / `gpt-5.5` review+implement.) **Also served on Anthropic `/v1/messages` via the translation shim (→ `/chat/completions`)** — see the shim section below. |
