...
| 2 image blocks to a `max_prompt_images: 1` model | `gpt-5.5` → `/responses` | ❌ 400 **locally**, before any upstream call | exploratory | `vision_over_limit_local_reject` | Asserts the outbound preflight converts what used to be an opaque upstream rejection into a local error naming the model and both counts. A `200` here would mean the preflight stopped running; an upstream-shaped `400` would mean it stopped running *and* the limit is no longer enforced. |
