...
| `gpt-5.6-luna` | ✅ 200 live-verified 2026-08-24 (`/v1/responses` function `tools[]` + `reasoning:{effort:"high"\|"max"}`) | exploratory | `fast_luna_responses_reasoning_high` / `fast_luna_responses_reasoning_max` | **Fast profile:** the fast-launch-profile lead, and the model behind its `Explore` (high) and `implementer` (max) assignments — see [`default-models.md`](default-models.md) "Fast launch profile". 1.05M context, `/v1/responses`-capable, full `none..max` effort ladder. |
