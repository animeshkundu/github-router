...
| `gpt-5.6-luna` | ✅ 200 live-verified 2026-08-24 (`/v1/responses` function `tools[]` + `reasoning:{effort:"high"\|"max"}`) | exploratory | `fast_luna_responses_reasoning_high` / `fast_luna_responses_reasoning_max` | **Fast profile:** the fast-launch-profile lead, and the model behind its `scout` (high) and `implementer` (max) assignments — see [`default-models.md`](default-models.md) "Fast launch profile". 1.05M context, `/v1/responses`-capable, full `none..max` effort ladder. |
| `gemini-3.7-flash` | ✅ 200 live-verified 2026-08-24 (`/v1/chat/completions` tools + `reasoning_effort:"high"|"medium"`; `/v1/messages` shim tool-use) | exploratory | `fast_gemini37flash_chat_reasoning_high` / `fast_gemini37flash_chat_reasoning_medium` / `fast_gemini37flash_messages_tool_use` | Standard reviewer/brainstorm fallback and `reviewer-fast`; fast native `critic` at medium and lead-only Advisor at high. `low..high` effort, 1M context. |
| `grok-4.6` | ✅ 200 live-verified 2026-08-24 (`/v1/responses` function `tools[]` + `reasoning:{effort:"medium"}`) | exploratory | `fast_grok46_responses_reasoning_medium` | Fast profile: native `reviewer` assignment. 500K total / **372K max-prompt** / 128K max-output, `low..xhigh` effort (no `max`). Kept bare (no `[1m]`) — see "Grok context accounting" in [`default-models.md`](default-models.md). Also served on Anthropic `/v1/messages` via the translation shim (→ `/responses`, generic `supported_endpoints` routing — see the shim section below, `shim_grok46_messages*`). |
