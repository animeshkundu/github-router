...
| Fast capitalized `Explore` | custom `Explore` → Luna/high with a bare/default-accounted private alias; no fast `scout` | ✅ deterministic agent-definition/ACL test + installed-client canary | proxy-internal | `fast_explore_fixed_model` | Replaces fast `scout` and intentionally shadows the client built-in only in `-m fast`. Luna's live catalog prerequisite remains ≥1M because Luna is also the fast lead and implementer; bare local accounting does not claim a 200K provider window. Standard lowercase `scout` and built-in Explore remain unchanged. |
| `gpt-5.6-sol` | ✅ 200 (`/v1/responses` accepts function `tools[]` + `reasoning:{effort:"high"|"xhigh"}`) | exploratory | `worker_gpt56sol_responses_tools_reasoning` / `fast_sol_responses_reasoning_high` | **Default/preferred** OpenAI model for the `implement`/`test` workers, standard native `implementer`, and codex CLI; fast `Plan` uses high effort with `[1m]` accounting. 1.05M total / 922K max-prompt / 128K max-output. Fast policy fixes it to Responses even if a future catalog advertises both endpoints. |
...
| `gpt-5.6-luna` | ✅ 200 live-verified 2026-08-24 (`/v1/responses` function `tools[]` + `reasoning:{effort:"high"\|"max"}`) | exploratory | `fast_luna_responses_reasoning_high` / `fast_luna_responses_reasoning_max` | **Fast profile:** the fast-launch-profile lead, and the model behind its `Explore` (high, bare/default local accounting) and `implementer` (max, `[1m]` accounting) assignments — see [`default-models.md`](default-models.md) "Fast launch profile". The bare Explore id does not claim Luna's provider window is only 200K. Live catalog: 1.05M total / 922K max-prompt / 128K max-output, `/v1/responses`-capable, full `none..max` effort ladder. |
| `gemini-3.7-flash` | ✅ 200 live-verified 2026-08-24 (`/v1/chat/completions` tools + `reasoning_effort:"high"|"medium"`; `/v1/messages` shim tool-use) | exploratory | `fast_gemini37flash_chat_reasoning_high` / `fast_gemini37flash_chat_reasoning_medium` / `fast_gemini37flash_messages_tool_use` | Standard reviewer/brainstorm fallback and `reviewer-fast`; fast native `critic` at medium and primary-lead-only Advisor at high across every fixed `/model` selection. `low..high` effort, 1M context. |
