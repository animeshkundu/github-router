...
|---|---|---|---|---|
| `claude-opus-5` (Anthropic dashed slug, default) | ✅ 200 (exact catalog match) | claude-emits | (used by every claude probe) | Single base slug, natively 1M; no `-1m` sibling |
...
| `gpt-5.6-sol` | ✅ 200 (`/v1/responses` accepts function `tools[]` + `reasoning:{effort:"xhigh"}`) | exploratory | `worker_gpt56sol_responses_tools_reasoning` | **Default/preferred** OpenAI model for the `implement`/`test` worker tools, the native `implementer` subagent, `advisor`, `codex_critic`, the `stand_in` OpenAI slot, first-mate tiers, and the codex CLI default. 1050k context, `/v1/responses`-only, restriction tier `pro_plus/business/enterprise/max` (same as gpt-5.5). Fallback behavior is surface-specific; see the per-surface docs. |
...
| `gemini-3.1-pro-preview` | ✅ 200 (`/v1/chat/completions` accepts `tools[]` + `reasoning_effort:"high"`) | exploratory | `shim_gemini31pro_messages` (+ shape carrier `worker_gemini_tools_reasoning`) | Model for the `gemini-critic` / `gemini-reviewer` personas, `stand_in`, and the review worker. Explore defaults to `gemini-3.6-flash`; implement/test default to `gpt-5.6-sol`. **Also served on Anthropic `/v1/messages` via the translation shim (→ `/chat/completions`)** — see the shim section below. |
...
| `gpt-5.6-luna` | ✅ 200 live-verified 2026-08-24 (`/v1/responses` function `tools[]` + `reasoning:{effort:"high"\|"max"}`) | exploratory | `fast_luna_responses_reasoning_high` / `fast_luna_responses_reasoning_max` | **Fast profile:** the fast-launch-profile lead, and the model behind its `scout` (high) and `implementer-fast` (max) assignments — see [`default-models.md`](default-models.md) "Fast launch profile". 1.05M context, `/v1/responses`-capable, full `none..max` effort ladder. |
| `gemini-3.7-flash` | ✅ 200 live-verified 2026-08-24 (`/v1/chat/completions` `tools[]` + `reasoning_effort:"high"`) | exploratory | `fast_gemini37flash_chat_reasoning_high` | Already the fallback for `reviewerModel()`/`brainstormModel()` and the single-entry `reviewer-fast` native. **In-progress design**: also the fast profile's sole persona (`gemini-critic`) and Advisor target. `low..high` effort (no `xhigh`), 1M context. |
| `grok-4.6` | ✅ 200 live-verified 2026-08-24 (`/v1/responses` function `tools[]` + `reasoning:{effort:"medium"}`) | exploratory | `fast_grok46_responses_reasoning_medium` | **In-progress design**: the fast profile's `reviewer-fast` assignment. 500K total / **372K max-prompt** / 128K max-output, `low..xhigh` effort (no `max`). Kept bare (no `[1m]`) — see "Grok context accounting" in [`default-models.md`](default-models.md). Also served on Anthropic `/v1/messages` via the translation shim (→ `/responses`, generic `supported_endpoints` routing — see the shim section below, `shim_grok46_messages*`). |
...
**`grok-4.6` (in-progress design, probe coverage added, not yet live-run by this document's author):** the `shim_grok46_messages` / `shim_grok46_messages_tool_use` probes exercise the same Responses shim path as `gpt-5.5` against the fast-launch-profile's Grok assignment. Because routing is generic (catalog `supported_endpoints`, no hardcoded slug list), these rows should pass against today's shim code once `grok-4.6` is present in the live catalog — but they have not been run against a live proxy for this document, so treat the row above as "probe exists, awaiting a live run" rather than "verified."
...
| `gemini-3.5-flash` | `/chat/completions` | ✅ 200 via shim | exploratory | `shim_gemini35flash_messages` / `shim_gemini35flash_messages_streaming` / `shim_gemini35flash_messages_tool_use` | Non-stream verified live; stream + tool-use **probe coverage added** (to be run live by the lead). No `xhigh` (reasoning clamps to `high`); no `structured_outputs`. 1M context native on the base slug. Tool-use probe uses `tool_choice:auto` (not forced — this model early-stops on forced tool-calls; autonomous auto-mode is the working pattern). |
| `gemini-3.1-pro-preview` | `/chat/completions` | ✅ 200 via shim | exploratory | `shim_gemini31pro_messages` | Same gemini gaps: no `xhigh` (clamps to `high`), no `structured_outputs`. 1M context native. |
| `grok-4.6` | `/responses` | ✅ 200 live-verified 2026-08-24 | exploratory | `shim_grok46_messages` / `shim_grok46_messages_tool_use` | **In-progress design** (shipped as a user-facing surface): "the translated Grok lead/tool path" the fast launch profile depends on for its `reviewer-fast` assignment. Routes through the SAME generic Responses shim as `gpt-5.5`/`gpt-5.6-sol` (`classifyMessagesRoute` keys off catalog `supported_endpoints`, never a hardcoded slug list) — no shim code change is required for this row to pass. 500K total / 372K max-prompt / 128K max-output, `low..xhigh` effort (no `max`); kept bare (no `[1m]`) — see "Grok context accounting" in [`default-models.md`](default-models.md). |
...
| 12 image blocks to a model whose real ceiling is 10 | `gemini-3.1-pro-preview` → `/chat/completions` | ✅ 200 | exploratory | `vision_ceiling_recovery_gemini` | Copilot names the real number when it refuses (`too many images: maximum allowed for model gemini-3.6-flash is 10, got 16`). The proxy parses it, prunes to it keeping the most recent images, retries **once**, and remembers it for the process lifetime; a second rejection or an unparseable one is forwarded untouched, so it cannot loop. **What this probe asserts is only the user-visible outcome** — a 200 and a well-formed message, which without the recovery would be a 400. The mechanism itself (exactly one upstream rejection, pruned to 10, exactly one retry, the ceiling then applied proactively without a round trip) is pinned by `tests/vision-preflight.test.ts`, and was observed live on 2026-08-10 in the proxy warn log: `Dropped 2 of 12 image(s) bound for gemini-3.1-pro-preview (10 kept)`. |
...
| `anthropic-beta: advisor-tool-2026-03-01` plus advisor tool entries | `gemini-3.5-flash` → `/chat/completions` | ✅ 200 (graceful degrade; advisor unavailable, no `400`) | exploratory | `shim_advisor_degrade_gemini35flash` | Same graceful-degrade contract on the chat shim path. |
