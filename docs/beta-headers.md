...
The router strips `context-1m-`, `skills-`, `files-api-`, `code-execution-`, `output-128k-`, and **`advisor-tool-`** from every outgoing `anthropic-beta` value — Copilot returns 400 ("unsupported beta header") on each. The strip list lives in `EXPLICITLY_STRIPPED_BETA_PREFIXES` (`src/lib/utils.ts`) — defensive deny-list that catches even future allowlist broadenings. 1M context for Opus is unlocked by selecting a 1M-capable model id (`claude-opus-4.8`, `claude-opus-4.7-1m-internal`, `claude-opus-4.6-1m`), not via a beta header.

The router also strips body-level `budget`, `output_config.schema` (and the GA-nested `output_config.format`), `betas` (array), `speed` (string), and `diagnostics` (object) from `/v1/messages` and `/v1/messages/count_tokens` (Phase B; Copilot 400s on each — verified live). The corresponding `anthropic-beta` headers (`task-budgets-`, `structured-outputs-`, `fast-mode-`, `cache-diagnosis-`) are preserved or dropped by the allowlist but never forwarded to a backend Copilot lacks; only the per-request enforcement/hint field is dropped. For Structured Outputs the proxy captures the schema first — from the pre-GA flat `output_config.schema`/`.type` or the GA-nested `output_config.format.{schema,type}` (flat takes precedence) — and re-injects it as a system-prompt instruction so the structured-output intent survives the strip (probe `output_config_format_ga_stripped`). (`speed: "fast"` is the fast-mode latency hint — Copilot has no fast-mode backend, so the body field is dropped and the hint is NOT enforced upstream; end-to-end coverage in probes `speed_fast_stripped` / `speed_fast_count_tokens_stripped`. `diagnostics` is the `cache-diagnosis-2026-04-07` cache-introspection field — Copilot has no cache-diagnostics backend, so the body field is dropped and no synthetic `cache_miss_reason` is generated; probe `cache_diagnostics_stripped`.)
