...
describe("discount factors", () => {
  test("locked rows match the snapshot analysis", () => {
    expect(discountFactorFor("gpt-6-sol")).toBe(0.28)
    expect(discountFactorFor("claude-opus-5.5")).toBe(0.62)
    ...
    expect(discountFactorFor("gpt-6-luna")).toBe(1.0)
...
describe("discountedUsdForSnapshot", () => {
  test("sol session: 12.42 credits × 0.28", () => {
    const r = discountedUsdForSnapshot(snapshot({ "gpt-6-sol": 12_420_000_000 }))
    expect(r.total).toBeCloseTo(0.034776, 12)
    expect(r.byModel["gpt-6-sol"]).toBeCloseTo(0.034776, 12)
    expect(r.capped).toBe(false)
  })

  test("luna long-context nano passes through at 1.0", () => {
    const r = discountedUsdForSnapshot(snapshot({ "gpt-6-luna": 5_000_000_000 }))
  ...
  test("multi-model sums per-model contributions", () => {
    const r = discountedUsdForSnapshot(
      snapshot({ "gpt-6-sol": 10_000_000_000, "grok-4.6": 10_000_000_000 }),
    )
  ...
  test("fat-finger factor can never overstate: cap binds, rows rescale", () => {
    const snap = snapshot({ "gpt-6-sol": 10_000_000_000 })
    const r = discountedUsdWithFactors(snap, {
      "gpt-6-sol": { factor: 2.8, source: "test" },
    })
    // Cap is 10cr × $0.01 = $0.10; raw would be $0.28.
    expect(r.capped).toBe(true)
    expect(r.total).toBeCloseTo(0.1, 12)
    expect(r.byModel["gpt-6-sol"]).toBeCloseTo(0.1, 12)
  })
...
describe("formatDiscountedCostTable", () => {
  const snap = snapshot(
    { "gpt-6-sol": 12_420_000_000, "grok-4.6": 6_670_000_000 },
    {
      "gpt-6-sol": { input: 178066254, cache_read: 8549020308, cache_write: 0, output: 18710695, other: 0 },
  ...
  test("header, rows sorted by spend desc, total sums", () => {
    ...
    expect(lines[3]).toContain("gpt-6-sol")
  ...
  test("write column appears only with writes", () => {
    ...
    const withWrites = snapshot(
      { "gpt-6-luna": 1_000_000_000 },
      { "gpt-6-luna": { input: 1, cache_read: 10, cache_write: 2, output: 1, other: 0 } },
    )
