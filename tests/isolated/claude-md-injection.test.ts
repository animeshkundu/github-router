...
test("operating-defaults directive: orchestrator posture + concrete excellence principles, self-compliant", () => {
  ...
  expect(OPERATING_DEFAULTS_DIRECTIVE).toContain(
    "Launch independent agents concurrently in a single message rather than serially",
  )
...
test("buildOperatingDefaultsDigest provides profile-specific summaries while standard matches export", () => {
  ...
  const fastDigestWithoutAstra = buildOperatingDefaultsDigest({ profile: "fast" })
  ...
  expect(fastDigestWithoutAstra).not.toContain("`astra`")
  expect(fastDigestWithoutAstra).not.toContain("`planner`")
  expect(fastDigestWithoutAstra).not.toContain("`critic`")

  const fastDigestWithAstra = buildOperatingDefaultsDigest({ profile: "fast", astraAvailable: true })
