...
test("(a) unset → defaults to gpt-6-sol when no catalog is available", () => {
  expect(resolveCloudAgentModel(undefined, null)).toBe(DEFAULT_CODEX_MODEL)
  expect(DEFAULT_CODEX_MODEL).toBe("gpt-6-sol")
})

test("(a) unset → defaults to gpt-6-sol when the catalog is an empty array", () => {
...
test("(a) unset + populated catalog containing the preferred default → returns the preferred default", () => {
  const catalog = [{ id: "gpt-6-sol" }, { id: "gpt-5.4" }]
  expect(resolveCloudAgentModel(undefined, catalog)).toBe("gpt-6-sol")
})

test("(a) unset + populated catalog MISSING the preferred default → walks fallback chain", () => {
  // gpt-6-sol not in catalog; first available fallback wins.
