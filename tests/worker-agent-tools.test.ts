...
describe("code_search", () => {
  ...
  test("tool description is semantic-first when enabled, lexical-only when disabled (--search)", async () => {
    ...
    try {
      ...
      const bluebird = __testExports.codeSearchTool(dir, false, true)
      expect(bluebird.label).toBe("Code search (Bluebird)")
      expect(bluebird.description).toContain("Bluebird-backed")
      expect(bluebird.description).toContain("no local fallback")
      expect(bluebird.description).not.toContain("ColBERT")
      const both = __testExports.codeSearchTool(dir, true, true)
      expect(both.label).toBe("Code search (Bluebird)")
      expect(both.description).toContain("Bluebird-backed")
      ...
      // Omitted flags default to lexical-only.
