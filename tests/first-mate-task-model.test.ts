...
test("defaults to gpt-6-sol when no model is chosen and no catalog is available", () => {
...
test("returns the default when it IS present in the catalog", () => {
  expect(
    resolveCloudAgentModel(undefined, [{ id: "gpt-6-sol" }, { id: "gpt-5.4" }]),
  ).toBe("gpt-6-sol")
})

test("walks the fallback chain when the preferred default is absent for the tier", () => {
  // gpt-6-sol not served on this catalog; the first available fallback wins.
