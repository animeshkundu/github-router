...
// 10 MiB cap on non-streaming /v1/messages response bodies. Anthropic responses
// with large tool_use blocks can legitimately reach several MB, but a multi-GB
// body is either a buggy upstream or a malicious one. Buffering it would OOM
// the proxy and crash all in-flight requests.
 ...
 * on cap exceeded.
 */
async function readResponseBodyCapped<T>(
  response: Response,
  routePath: string,
  capBytes: number,
      ...
      errorResponse: {
        ...
        error: {
          ...
          message:
            ...
            + `/v1/messages. The upstream may be misbehaving. Try enabling streaming `
