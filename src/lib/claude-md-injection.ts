...
export function buildOperatingDefaultsDirective(
  ...
  if (opts.profile === "max") {
    return (
      ...
      + "Max launch profile. Delegate only when work is wide or slow; use `Explore` for broad read-only discovery, `Plan` for an evidence-grounded plan, `implementer` for coding that needs judgment, `general-purpose` for catch-all implementation, `reviewer` for repository-aware verification, and `brainstorm` before an approach is chosen. Each role's configured model is the deliberate default for intelligence, quality, speed, and cross-lab diversity; use it first and override only after a concrete failure or task-model mismatch, never speculatively. On every retained max surface, Gemini 3.1 Pro is replaced by Grok 4.6/high when available, otherwise Gemini 3.7 Flash 1M/high. `peer-review-coordinator` fans out to the max-profile peer critics when a consequential plan or diff needs independent review. The lead keeps decision ownership, runs relevant tests before declaring done, and uses only capabilities actually listed in this session."
    )
  }
  if (opts.profile === "fast") {
