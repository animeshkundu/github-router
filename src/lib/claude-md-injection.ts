...
export const OPERATING_DEFAULTS_DIRECTIVE =
  ...
  + "right subagent, worker, or model (the `implementer`, `debugger`, and `qa-engineer` "
  + "native subagents for bounded implementation, root-cause debugging, and "
  + "review/testing respectively; worker-* agents for background non-blocking runs; "
  + "Task subagents for parallel work; peer critics for review) so your own "
  ...
  + "context. Delegate to a subagent by default whenever one can do the work without "
  + "loss of quality: the main thread is where you think with and respond to the user, "
  + "and its context window is a finite shared resource, so protect it by pushing heavy "
  + "or exploratory work outward and keeping only the conclusions.\n\n"
...
export const OPERATING_DEFAULTS_DIGEST =
  ...
  + "worker, or model by default wherever one can do the work without losing quality, preferring "
  + "parallel delegation for independent work, to protect the main thread's finite context and keep it "
  + "free for reasoning and interacting with the user; do trivial, surgical, and last-mile work directly. "
