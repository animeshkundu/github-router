 ...
 *     the single gpt-5.6-luna scope/goal call the prompt hook makes.
 *
...
/**
 * One non-streaming inference via `/v1/responses` (model id is caller-chosen;
 * the prompt hook uses gpt-5.6-luna). Returns the assistant text (possibly
 * empty). Throws on transport/HTTP/parse failure. `effort` maps to the
 * Responses `reasoning.effort` knob.
 */
