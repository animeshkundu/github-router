 ...
 * `GH_ROUTER_AIC_USER_STATUSLINE` (set by the caller from the returned
 * `userCommand`) and the hook composes one line: `[AIC x.xx] <user line>`.
 * Any other pre-existing `statusLine` shape is left untouched
 * (`reason: "unrecognized"`) rather than risk breaking an exotic config.
 *
...
export type AicStatuslineInjectionResult =
  | { written: true; mode: "injected" | "wrapped"; userCommand?: string }
