...
import { isNaturalLanguageQuery, rerankDocuments } from "~/lib/rerank"
import { acquireSearchSlot, recommendedRgThreads } from "~/lib/search-concurrency";
...
/**
 * Head size for the JIT cross-encoder rerank (Phase 3). Only the first
 * RERANK_HEAD BM25F hits are reranked; beyond that BM25F order stands.
 * Sized so both models fit the rerank budget: tiny ≈12ms, mini ≈120ms
 * (both under the 150ms default). Covers the window a model actually
 * reads; deeper hits keep their BM25F relevance order.
 */
const RERANK_HEAD = 25;
 ...
 * fired. Keywords are deliberately cross-language (`struct` also
 * catches C definitions, `namespace` C++/TS ones); each requires
 * `keyword + identifier`, so incidental uses (`x := struct {`, assignments)
 * don't match.
 */
const SYMBOL_REGEX =
  /^(?:export\s+)?(?:default\s+)?(?:async\s+)?(?:public\s+|private\s+|protected\s+|static\s+|abstract\s+|readonly\s+|sealed\s+|partial\s+|internal\s+)*(?:function|class|interface|type|enum|def|fn|trait|impl|module|namespace|record|struct|delegate|event|const|let|var)\s+[A-Za-z_$]/;
...
export interface CodeSearchResponse {
  ...
  elapsed_ms: number;
  ranking: {
    algorithm: "BM25F" | "BM25F+FlashRank" | "ripgrep_document_order";
    citation?: string;
    k1?: number;
  };
  /**
...
function buildRgArgs(input: {
  ...
  // Thread budget (Phase 1, A5): size ripgrep's own thread pool from LIVE
  // search concurrency. A sole searcher gets all cores (rg default
  // behavior); N concurrent searches divide them so 16 parallel queries
  // don't each fan out to `cpus` threads and thrash a 4-core box.
  // Evaluated at spawn time — after the A8 slot is held, so the count
  // includes this call.
  args.push("-j", String(recommendedRgThreads()));
  ...
  if (
    !input.expansionPattern &&
    (input.mode === "literal" || input.mode === "ranked")
  ) {
    args.push("-F");
  }
    ...
    for await (const rawLine of rl) {
      ...
      switch (evt.type) {
        ...
        case "context": {
          ...
          if (
            lastHitForContext &&
            lastHitForContext.context_after.length < opts.contextLines
          ) {
 ...
 */
/**
 * Bounded concurrency for the structural-pass pre-read (Phase 1 below).
 * Disk reads overlap; parses stay serial (single WASM heap). 8 keeps a
 * slow network FS from fanning out unboundedly while still collapsing N
 * serial round-trips into ~one batch on typical top-N sets (≤50 files).
 */
const STRUCTURAL_PREREAD_CONCURRENCY = 8;

/** One file's Phase-1 (I/O) outcome, consumed by the serial parse loop. */
type StructuralFileInput =
  | { kind: "skip" }
  | { kind: "cached"; cached: CachedTree }
  | { kind: "fresh"; mtimeMs: number; source: string }
  | { kind: "readfail"; mtimeMs: number };

async function runStructuralPassInProcess(opts: {
  byFile: Map<string, Array<{ hit: RawHit; index: number }>>;
  grammars: Map<string, Language>;
  workspaceRoot: string;
  cap: number;
  ...
  // Phase 1: concurrent stat + read. Parses must serialize (single WASM
  // heap), but the stat/readFile I/O feeding them need not — batch the disk
  // round-trips, then parse serially from memory in Phase 2. The wall-clock
  // budget gates the parse loop only, so a slow batch-read can't silently
  // consume it. Order-preserving: inputs align 1:1 with byFile order, so
  // the parse loop below sees the identical sequence as before.
  const fileList = [...byFile];
  const inputs: Array<StructuralFileInput> = new Array(fileList.length);
  {
    const tasks: Array<() => Promise<StructuralFileInput>> = fileList.map(
      ([relFile]) =>
        async (): Promise<StructuralFileInput> => {
          const langKey = getLanguageKeyForPath(relFile);
          if (!langKey) return { kind: "skip" }; // unsupported — silent skip
          if (!grammars.get(langKey)) return { kind: "skip" }; // no grammar — silent skip
          const absPath = path.join(opts.workspaceRoot, relFile);
          let mtimeMs: number;
          let size: number;
          try {
            // No signal: stat is metadata-only and fast; the worker-loop
            // abort check above plus the Phase-2 gate cover cancellation.
          ...
          // Cache hit → no read needed; the parse loop reuses the tree.
          const cached = cacheGet(absPath, mtimeMs);
          ...
          try {
            ...
            return { kind: "fresh", mtimeMs, source };
          } catch (err) {
            if (opts.signal.aborted) return { kind: "skip" };
            consola.debug(
              `[code_search] structural skip ${relFile} (read failed: ${(err as Error).message})`,
            );
            return { kind: "readfail", mtimeMs };
          }
        },
    );
    let next = 0;
    const workers = Array.from(
      { length: Math.min(STRUCTURAL_PREREAD_CONCURRENCY, tasks.length) },
      async () => {
        while (true) {
          ...
          inputs[i] = await tasks[i]();
        }
      },
    );
    await Promise.all(workers);
    // Slots never reached after an abort resolve as skips.
    for (let i = 0; i < inputs.length; i++) inputs[i] ??= { kind: "skip" };
  }

  // Phase 2: serial parse + confirm, budget-gated exactly as before.
  const t0 = Date.now();
  try {
    for (let fi = 0; fi < fileList.length; fi++) {
      ...
      const [relFile, entries] = fileList[fi];
      ...
      if (input.kind === "cached") {
        cached = input.cached;
      } else if (input.kind === "readfail") {
        cachePut(absPath, { mtimeMs: input.mtimeMs, tree: null, source: null });
        continue;
      } else {
...
function bm25fScore(
  ...
  const fileTokensByField: Record<
    keyof FieldTexts,
  ...
  for (const qt of queryTokens) {
    ...
    for (const fname of Object.keys(fileTokensByField) as Array<
      keyof FieldTexts
    >) {
  ...
  for (const fname of Object.keys(avglen) as Array<keyof FieldTexts>) {
    ...
    avglen[fname] =
      lens.length > 0 ? lens.reduce((a, b) => a + b, 0) / lens.length : 1;
  ...
  for (let i = 0; i < hits.length; i++) {
    ...
    for (const qt of queryTokens) {
      ...
      for (const fname of Object.keys(FIELD_BOOSTS) as Array<
        keyof FieldTexts
      >) {
...
function renderSnippet(hit: RawHit): string {
  const lines = [...hit.context_before, hit.matched_line, ...hit.context_after];
...
export async function searchCode(
  ...
  // Bound active search weight across parallel calls (Phase 1, A8). The
  // slot covers generation + ranking + outlines (the whole CPU-bound body)
  // and releases on every exit path via the trailing finally. Acquired
  // AFTER validation so invalid calls fail fast without holding a slot.
  const slotMode = astPattern
    ? "ast"
    : mode === "ranked"
      ? "ranked"
      : mode === "literal"
        ? "literal"
        : "regex";
  const releaseSlot = await acquireSearchSlot(slotMode, ac.signal);
  try {
    ...
    // Set when the JIT cross-encoder rerank reordered the head (Phase 3).
    ...
    if (effectiveMode === "ranked") {
      ...
      // JIT cross-encoder rerank (Phase 3): NL-shaped queries only, never
      // under `complete:true` (exhaustive mode keeps BM25F order). Refines
      // the head (first RERANK_HEAD hits) — beyond that BM25F order stands.
      // Null (disabled/unavailable/over-budget) keeps BM25F order silently;
      // reranking is precision, never correctness.
      if (
        !rawInput.complete &&
        kept.length > 1 &&
        isNaturalLanguageQuery(rawInput.query)
      ) {
        const head = kept.slice(0, RERANK_HEAD)
        const order = await rerankDocuments(
          rawInput.query,
          head.map((sh) => renderSnippet(sh.hit)),
          head.length,
          ac.signal,
        )
        if (order && order.length === head.length) {
          const reordered = order.map((i) => head[i])
          kept = [...reordered, ...kept.slice(head.length)]
          rerankApplied = true
        }
      }
    ...
    // separately attributes the outline pass in the log line below.
    let outlines:
      Array<{ file: string; outline: Array<FileOutlineEntry> }> | undefined;
    ...
    const mergedNotice =
      finalNotices.length > 0 ? finalNotices.join(" · ") : null;

    outline_ms = Date.now() - outlineStartMs;
    ...
    consola.info(
      `[code_search] mode=${effectiveMode}${astPattern ? " ast_pattern" : ""}` +
        ...
        (debugLog
          ? ` query="${rawInput.query}" workspace="${ws.canonical}"`
          : ""),
    );
    ...
    const response: CodeSearchResponse = {
      ...
      ranking:
        effectiveMode === "ranked"
          ? {
              algorithm: rerankApplied ? "BM25F+FlashRank" : "BM25F",
