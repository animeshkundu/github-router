...
export function buildPeerAwarenessSnippet(opts: {
  ...
  if (opts.profile === "fast" || opts.profile === "cheap" || opts.profile === "cheap1m" || opts.profile === "cheapest" || opts.profile === "balanced") {
    ...
    const balancedRosterClause = "Native Task roster: `Explore` (targeted breadth only when search is insufficient, launch in parallel; lead/`Plan` narrow scope with search first), `Plan` (sequencing, interfaces, migration risk, acceptance criteria in plan mode; delegate discovery to `Explore` and write handoff-ready steps for `General-Purpose`), `General-Purpose` (mixed execution; follows a Plan handoff when one exists and drives to a verified end state), and `reviewer` (repo-aware verification after non-trivial changes, before done; narrows scope with search first and may invoke `Explore` for targeted discovery). In plan mode, delegate planning to `Plan` and do not edit files. Verify claims with concrete repository evidence and tests before declaring done."
    ...
    const escalationClause = isBalanced
      ? `This is the ${profileLabel} launch profile. Funnel unknowns cheapest-first: settle factual claims with \`mcp__${fastSearchKey}__code\` + \`mcp__${fastSearchKey}__web\` search first (narrow scope, rule out hypotheses), then \`Explore\` for targeted breadth, then \`Plan\`. \`mcp__${fastPeersKey}__oracle\` is ${oracleDescriptor}, a second opinion for precise, self-contained architectural/spec trade-offs that search, \`Explore\`, and \`Plan\` cannot settle, available to the lead and \`Plan\`; \`reviewer\` and other subagents cannot call Oracle.${astraClause}`
