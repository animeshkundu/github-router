...
export function validateIndexIntegrity(projectIndexDir: string): IndexIntegrity {
  ...
  for (const interval of intervals) {
    ...
    if (interval.start > cursorEnd) return { verdict: "corrupt", reason: "gap between shard intervals" }
    cursorEnd = interval.start + interval.count
    sum += interval.count
  }
  if (cursorEnd !== sum) {
    return { verdict: "corrupt", reason: "covered embedding range differs from shard total" }
  }
...
export async function freshnessVerdict(workspace: string): Promise<{
  ...
  // binary/runtime bits is not safe to label semantic-ready.
