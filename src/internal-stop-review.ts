 ...
 * HTTP (gpt-6-luna at high thinking, the live working tree, an accountability
...
export const internalStopReview = defineCommand({
  meta: {
    ...
    description:
      ...
      + "gpt-6-luna review of the working tree against the user's ask, and writes advisory findings for the next "
  ...
  async run() {
    try {
      ...
      const result = await callMcpTool({
        ...
        group: "workers",
        tool: "review",
        args: { prompt: brief, workspace: cwd, model: "gpt-6-luna", thinking: "high" },
        timeoutMs: REVIEW_TIMEOUT_MS,
      })

      // Only persist a usable, non-error review. An error envelope (e.g. gpt-6-luna
