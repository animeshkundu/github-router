 ...
 * HTTP (gpt-5.6-luna at high thinking, the live working tree, an accountability
 * brief: judge the diff against the user's actual ask for wrong-spec /
 * vacuous-tests / incompleteness), and writes the findings to the per-session
 * findings file. The next `UserPromptSubmit` surfaces them to Claude
 * NON-authoritatively, and the user sees them as an accountability log.
 *
...
export const internalStopReview = defineCommand({
  meta: {
    ...
    description:
      ...
      + "gpt-5.6-luna review of the working tree against the user's ask, and writes advisory findings for the next "
  ...
  async run() {
    try {
      ...
      const result = await callMcpTool({
        ...
        group: "workers",
        tool: "review",
        args: { prompt: brief, workspace: cwd, model: "gpt-5.6-luna", thinking: "high" },
        timeoutMs: REVIEW_TIMEOUT_MS,
      })

      // Only persist a usable, non-error review. An error envelope (e.g. gpt-5.6-luna
