// VENDOR PATCH (github-router): these provider option types are type-only
// metadata. Their upstream definitions pull in concrete provider SDKs that the
// vendored worker slice intentionally excludes because all traffic uses our
// custom Copilot streamFn. Keep the API-keyed option map without those deps.
type AnthropicOptions = Record<string, unknown>;
type AzureOpenAIResponsesOptions = Record<string, unknown>;
type BedrockOptions = Record<string, unknown>;
type GoogleOptions = Record<string, unknown>;
type GoogleVertexOptions = Record<string, unknown>;
type MistralOptions = Record<string, unknown>;
type OpenAICodexResponsesOptions = Record<string, unknown>;
type OpenAICompletionsOptions = Record<string, unknown>;
type OpenAIResponsesOptions = Record<string, unknown>;
type PiMessagesOptions = Record<string, unknown>;
